...
public class ShotgunShoot : UdonSharpBehaviour
{
    ...
    public void Shoot()
    {
        ...
        if (Physics.Raycast(barrel.position, barrel.TransformDirection(direction2 * Range), out RaycastHit HitData2, Range)) //Check to see if Ray hit any colliders
        {
            //Leave blank until Enemy AI is implemented on their own layer
        }

        if (Physics.Raycast(barrel.position, barrel.TransformDirection(direction3 * Range), out RaycastHit HitData3, Range)) //Check to see if Ray hit any colliders
        {
            //Leave blank until Enemy AI is implemented on their own layer
        }

        if (Physics.Raycast(barrel.position, barrel.TransformDirection(direction4 * Range), out RaycastHit HitData4, Range)) //Check to see if Ray hit any colliders
        {
            //Leave blank until Enemy AI is implemented on their own layer
        }

        if (Physics.Raycast(barrel.position, barrel.TransformDirection(direction5 * Range), out RaycastHit HitData5, Range)) //Check to see if Ray hit any colliders
        {
            //Leave blank until Enemy AI is implemented on their own layer
        }
    }
